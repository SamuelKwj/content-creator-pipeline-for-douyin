...
c) 不找 → state 标 `benchmark_status: none`，用通用 v0 起步
   - **Valid reasons**: your content voice is genuinely hybrid/synthetic, you "我就是我自己", no single benchmark account fits
   - This is NOT a degraded path — many creators ARE their own benchmark. The rubric calibrates from YOUR data after 5+ publish-retro cycles. Cold-start with no benchmark just means more upfront data collection before the rubric becomes useful for decisions.
...
2. **`rubric_notes.md`**
   ...
   ```
   - 复制 `starter-rubrics/<form>-zero.md`（cold-start）或 `<form>.md`（已有数据时仍可参考）
...
2.5. **`rubric-memo.md`**（**新**——配合 cheat-score-blind 隔离协议）
   ...
   ```
   - 复制 `templates/rubric-memo.template.md` → `<user-repo>/rubric-memo.md`
...
3. **`script_patterns.md`**
   ...
   ```
   - 复制 `templates/script_patterns.template.md`
...
4.5. **`benchmark.md`**（仅 Phase 2.5 选 a/b 时）
   ...
   ```
   - 复制 `templates/benchmark.template.md` → `<user-repo>/benchmark.md`
...
4.7. **`audience.md`**
   ...
   ```
   - 复制 `templates/audience.template.md` → `<user-repo>/audience.md`
