...
> 这是 `/cheat-init` 在你的项目根创建的速查文档。完整规范在 `SKILL.md` 和 `references/shared-references/`。
...
bump 是高风险操作——会做 5 步验证（含跨模型独立审）。详见 `references/shared-references/bump-validation-protocol.md`。
...
- 看 `SKILL.md` 的"必须拒绝的请求"段——你想做的事可能正好是被设计拒绝的
- 看对应子 skill 的 `skill/cheat-X/SKILL.md`
