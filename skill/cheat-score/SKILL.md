...
## 🔥 硬性规定：所有打分必须盲打（sub-agent 隔离）

**规则来源：** 校准实验中发现主 agent 因对话上下文和历史数据存在信息污染，inline 评分不客观。由此产生的硬性规则：

> "以后所有评分都得通过 sub-agent 盲打"

**由此产生的硬性规则：**

| 规则 | 说明 |
|---|---|
| 🚫 **主 agent 绝不 inline 评分** | 任何维度的 0-5 分都不能由主 agent 给出。评分是 sub-agent 的专属工作 |
| 🧑‍⚖️ **主 agent 只做调度+展示** | 调 sub-agent → sub-agent 返回 JSON → 主 agent 格式化输出，不修改分数 |
| 🔒 **无例外** | 即使用户说"你直接打吧"或"跳过 blind"——拒绝。这是校准循环的核心保障 |
| 📋 **违规后果** | 主 agent 若 inline 评分，用户视为校准循环故障，必须重建信任 |
...
用户: "打分这篇 [path]"
  ↓
主 agent: 读 [path] + rubric_notes.md
  ↓
主 agent → delegate_task(goal="按rubric盲打分...", context=[只含脚本+rubric], toolsets=["file"])
  ↓
sub-agent 返回 JSON → 主 agent 只做展示
```
...
- 「跳过 blind sub-agent 让主 Claude 直接打」 → cheat-score **坚决拒绝**。即使用户主动要求跳过盲打，也必须解释这是校准循环的核心保障。无任何 escape hatch，包括 `--skip-blind`。如果 Task tool 不可用 → 停止工作，等待工具恢复，不降级 inline 打分

## Post-Score: Dimension-Specific Rewrite Coaching

当 sub-agent 评分返回后，用户可能说"XX维度分数低，怎么改"——这是正常的后续行为。此时：

1. **consult `references/dimension-improvement-guide.md`** for per-dimension rewrite techniques
2. 给每个低分维度（≤3）提供 1-2 个具体改写建议，含改前/改后示例
3. **不改用户全文**——只给针对性示例，用户自己集成到稿子里
4. 建议完后问"要再打一次分吗"

如果用户说"我就按这个发"→ 跳过分段，直接建议 cheat-predict。
...
- `references/dimension-improvement-guide.md` — 低分维度改写技巧参考，post-score 可查
