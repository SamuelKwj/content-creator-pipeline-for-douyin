...
- **Stale in_progress_session**: When running cheat-predict for a new article while `.cheat-state.json` has `in_progress_session` pointing to a different (unpublished) article, simply overwrite the field. The old prediction file is already persisted at `predictions/`, so no data is lost. Do not attempt to maintain a history of old entries — `in_progress_session` is a single-entry pointer.
